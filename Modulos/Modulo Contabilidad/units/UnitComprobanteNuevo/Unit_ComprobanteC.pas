...
var
  ...
  _idPersona, _nombre: String;
  _esValido: Boolean;
  _valor, _valorReteFuente, _valorReteICA, _tasaReteFuente, _tasaReteICA : Currency;
  _valores: TRDsaValorArray;
  _rdsaValor: TRDsaValor;
begin
        CDSauxiliargrid.First;
        _esValido := false;
        _valor := 0;
        while not CDSauxiliargrid.Eof do
        begin
            if (CDSauxiliargridID_IDENTIFICACION.Value > 0 ) then
            begin
                _idIdentificacion :=  CDSauxiliargridID_IDENTIFICACION.Value;
                _idPersona := CDSauxiliargridID_PERSONA.Value;
                _nombre := CDSauxiliargridNOMBRE.Value + ' ' + CDSauxiliargridPRIMER_APELLIDO.Value + ' ' + CDSauxiliargridSEGUNDO_APELLIDO.Value;
            end;
            CDSauxiliargrid.Next;
        end;

        frmDocumentoSoporte := TfrmDocumentoSoporte.Create(nil);
        frmDocumentoSoporte.tipoComprobante := dblcbtipo.KeyValue;
        frmDocumentoSoporte.idComprobante := StrToInt(vid_comprobante);
        frmDocumentoSoporte.idIdentificacion := _idIdentificacion;
        frmDocumentoSoporte.idPersona := _idPersona;
        frmDocumentoSoporte.nombre := _nombre;
        frmDocumentoSoporte.descripcionDSA := editdescripcion.Text;
        frmDocumentoSoporte.fechaDSA := edFecha.Date;
        frmDocumentoSoporte.data := CDSauxiliargrid;
