...
uses
  ...
  // Propias
  UnitPlantillas, ComCtrls, UnitdmGeneral, IBStoredProc
  ;
...
begin
        ...
        IBQinsauxiliarext.Database := dmGeneral.IBDatabase1;

        IBQinsauxiliar.Transaction := dmGeneral.IBTransaction1;
        IBQinsauxiliarext.Transaction := dmGeneral.IBTransaction1;
...
begin
      ...
      IBQinscomprobante.Transaction := _transaccion;
      IBQinsauxiliar.Transaction := _transaccion;
      IBQinsauxiliarext.Transaction := _transaccion;
        try
         IBQinscomprobante.Close;
         IBQinscomprobante.sql.Clear;
         IBQinscomprobante.sql.Add('insert into "con$comprobante" ("con$comprobante"."ID_COMPROBANTE",');
         IBQinscomprobante.sql.Add('"con$comprobante"."FECHADIA", "con$comprobante"."TIPO_COMPROBANTE",');
         IBQinscomprobante.sql.Add('"con$comprobante"."ID_AGENCIA", "con$comprobante"."DESCRIPCION",');
         IBQinscomprobante.sql.Add('"con$comprobante"."TOTAL_DEBITO", "con$comprobante"."TOTAL_CREDITO",');
         IBQinscomprobante.sql.Add('"con$comprobante"."ESTADO", "con$comprobante"."IMPRESO",');
         IBQinscomprobante.sql.Add('"con$comprobante"."ANULACION","con$comprobante".ID_EMPLEADO)');
         IBQinscomprobante.sql.Add('values (');
         IBQinscomprobante.sql.Add(':"ID_COMPROBANTE", :"FECHADIA", :"TIPO_COMPROBANTE",');
         IBQinscomprobante.sql.Add(':"ID_AGENCIA", :"DESCRIPCION", :"TOTAL_DEBITO",');
         IBQinscomprobante.sql.Add(':"TOTAL_CREDITO", :"ESTADO", :"IMPRESO", :"ANULACION",:ID_EMPLEADO)');

         IBQinscomprobante.ParamByName('ID_COMPROBANTE').AsString:= vid_comprobante;
         IBQinscomprobante.ParamByname('FECHADIA').AsDate := edFecha.Date;
         IBQinscomprobante.ParamByName('ID_AGENCIA').AsInteger := Agencia;
         IBQinscomprobante.ParamByName('TIPO_COMPROBANTE').AsInteger := dblcbtipo.KeyValue;
         IBQinscomprobante.ParamByName('DESCRIPCION').AsBlob := vdescripcion;
         IBQinscomprobante.ParamByName('TOTAL_DEBITO').AsCurrency  := vtotal_debito;
         IBQinscomprobante.ParamByName('TOTAL_CREDITO').AsCurrency  := vtotal_credito;
         IBQinscomprobante.ParamByName('ESTADO').AsString  := 'O';
         IBQinscomprobante.ParamByname('ANULACION').asstring := '';
         IBQinscomprobante.ParamByName('IMPRESO').AsInteger  := Ord(false);
         IBQinscomprobante.ParamByName('ID_EMPLEADO').AsString := DBAlias;
         IBQinscomprobante.ExecSQL;

         IBQinsauxiliar.SQL.Clear;
         IBQinsauxiliar.SQL.Add('insert into "con$auxiliar" (');
         IBQinsauxiliar.SQL.Add('"ID_COMPROBANTE","ID_AGENCIA","FECHA","CODIGO","DEBITO",');
         IBQinsauxiliar.SQL.Add('"CREDITO","ID_CUENTA","ID_COLOCACION","ID_IDENTIFICACION",');
         IBQinsauxiliar.SQL.Add('"ID_PERSONA","MONTO_RETENCION","TASA_RETENCION","ESTADOAUX","TIPO_COMPROBANTE","ID")');
         IBQinsauxiliar.SQL.Add('values (');
         IBQinsauxiliar.SQL.Add(':"ID_COMPROBANTE",:"ID_AGENCIA",:"FECHA",:"CODIGO",:"DEBITO",');
         IBQinsauxiliar.SQL.Add(':"CREDITO",:"ID_CUENTA",:"ID_COLOCACION",:"ID_IDENTIFICACION",');
         IBQinsauxiliar.SQL.Add(':"ID_PERSONA",:"MONTO_RETENCION",:"TASA_RETENCION",:"ESTADOAUX",:"TIPO_COMPROBANTE",:"ID")');
         ...
         begin
            ...
            IBQinsauxiliar.ParamByName('TIPO_COMPROBANTE').AsInteger :=  dblcbtipo.KeyValue;
            IBQinsauxiliar.ParamByName('ID_COMPROBANTE').AsString := vid_comprobante;
            IBQinsauxiliar.ParamByName('ID_AGENCIA').AsInteger:= Agencia;
            IBQinsauxiliar.ParamByName('FECHA').AsDate := edFecha.Date;
            IBQinsauxiliar.ParamByName('CODIGO').AsString := Arecord^.codigo;
            IBQinsauxiliar.ParamByName('DEBITO').AsCurrency := Arecord^.debito;
            IBQinsauxiliar.ParamByName('CREDITO').AsCurrency := Arecord^.credito;
            IBQinsauxiliar.ParamByName('ID_CUENTA').Clear;
            IBQinsauxiliar.ParamByName('ID_COLOCACION').Clear;
            IBQinsauxiliar.ParamByName('ID_IDENTIFICACION').AsInteger := Arecord^.tipoide;
            IBQinsauxiliar.ParamByName('ID_PERSONA').AsString := Arecord^.idpersona;
            IBQinsauxiliar.ParamByName('MONTO_RETENCION').AsCurrency := Arecord^.monto;
            IBQinsauxiliar.ParamByName('TASA_RETENCION').AsFloat := Arecord^.tasa;
            IBQinsauxiliar.ParamByName('ESTADOAUX').AsString := 'O';
            IBQinsauxiliar.ParamByName('ID').AsInteger := idauxiliar;
...
begin
      ...
      IBQinsauxiliarext.Transaction := _transaccion;
       try
         IBQinscomprobante.sql.Clear;
         IBQinscomprobante.sql.Add('update "con$comprobante" set ');
         IBQinscomprobante.sql.Add('"con$comprobante"."DESCRIPCION" =:"DESCRIPCION",');
         IBQinscomprobante.sql.Add('"con$comprobante"."TOTAL_DEBITO" =:"TOTAL_DEBITO",');
         IBQinscomprobante.sql.Add('"con$comprobante"."TOTAL_CREDITO" =:"TOTAL_CREDITO",');
         IBQinscomprobante.sql.Add('"con$comprobante"."ESTADO" =:"ESTADO",');
         IBQinscomprobante.sql.Add('"con$comprobante"."IMPRESO" =:"IMPRESO",');
         IBQinscomprobante.sql.Add('"con$comprobante"."ANULACION" =:"ANULACION",');
         IBQinscomprobante.sql.Add('"con$comprobante".ID_EMPLEADO = :ID_EMPLEADO,');
         IBQinscomprobante.sql.Add('"con$comprobante".FECHADIA = :FECHADIA');
         IBQinscomprobante.sql.Add(' where ');
         IBQinscomprobante.sql.Add('"con$comprobante"."TIPO_COMPROBANTE" = :"TIPO_COMPROBANTE" and');
         IBQinscomprobante.sql.Add('"con$comprobante"."ID_COMPROBANTE" = :"ID_COMPROBANTE" and');
         IBQinscomprobante.sql.Add('"con$comprobante"."ID_AGENCIA" = :"ID_AGENCIA"');

         IBQinscomprobante.ParamByName('TIPO_COMPROBANTE').AsInteger := dblcbtipo.KeyValue;
         IBQinscomprobante.ParamByName('ID_COMPROBANTE').AsString:= vid_comprobante;
         IBQinscomprobante.ParamByName('FECHADIA').AsDate := edFecha.Date;
         IBQinscomprobante.ParamByName('ID_AGENCIA').AsInteger := Agencia;
         IBQinscomprobante.ParamByName('DESCRIPCION').AsString := vdescripcion;
         IBQinscomprobante.ParamByName('TOTAL_DEBITO').AsCurrency  := vtotal_debito;
         IBQinscomprobante.ParamByName('TOTAL_CREDITO').AsCurrency  := vtotal_credito;
         IBQinscomprobante.ParamByName('ESTADO').AsString  := 'O';
         IBQinscomprobante.ParamByName('IMPRESO').AsInteger  := Ord(False);
         IBQinscomprobante.ParamByName('ANULACION').AsString := anulacion;
         IBQinscomprobante.parambyname('ID_EMPLEADO').AsString := DBAlias;
         IBQinscomprobante.ExecSQL;

         IBQinsauxiliar.SQL.Clear;
         IBQinsauxiliar.SQL.Add('delete from "con$auxiliar" where ');
         IBQinsauxiliar.SQL.Add('"con$auxiliar"."TIPO_COMPROBANTE" = :"TIPO_COMPROBANTE" and');
         IBQinsauxiliar.SQL.Add('"con$auxiliar"."ID_COMPROBANTE" = :"ID_COMPROBANTE" and');
         IBQinsauxiliar.SQL.Add('"con$auxiliar"."ID_AGENCIA" = :"ID_AGENCIA"');
         IBQinsauxiliar.ParamByName('TIPO_COMPROBANTE').AsInteger := dblcbtipo.KeyValue;
         IBQinsauxiliar.ParamByName('ID_COMPROBANTE').AsString:= vid_comprobante;
         IBQinsauxiliar.ParamByName('ID_AGENCIA').AsInteger := Agencia;
         IBQinsauxiliar.ExecSQL;
         ...
         IBQinsauxiliar.SQL.Clear;
         IBQinsauxiliar.SQL.Add('delete from "con$auxiliarext" where ');
         IBQinsauxiliar.SQL.Add('"con$auxiliarext"."TIPO_COMPROBANTE" = :"TIPO_COMPROBANTE" and');
         IBQinsauxiliar.SQL.Add('"con$auxiliarext"."ID_COMPROBANTE" = :"ID_COMPROBANTE" and');
         IBQinsauxiliar.SQL.Add('"con$auxiliarext"."ID_AGENCIA" = :"ID_AGENCIA"');
         IBQinsauxiliar.ParamByName('TIPO_COMPROBANTE').AsInteger := dblcbtipo.KeyValue;
         IBQinsauxiliar.ParamByName('ID_COMPROBANTE').AsString:= vid_comprobante;
         IBQinsauxiliar.ParamByName('ID_AGENCIA').AsInteger := Agencia;
         IBQinsauxiliar.ExecSQL;
      ...
      _query2.Transaction := _transaccion;
        ...
        with _query2 do
          begin
                List.Clear;
                SQL.Clear;
                SQL.Add('select a.*,e.*, p.NOMBRE from "con$auxiliar" a inner join "con$puc" p on ');
                SQL.Add(' (a.CODIGO = p.CODIGO) ');
                SQL.Add(' left join "con$auxiliarext" e on a.ID = e.ID ');
                ...
                if (auxiliar <> 0) then
                 begin
                    ...
                    ARecord^.detalle := fieldbyname('DETALLE').AsString;
                    ARecord^.cheque := fieldbyName('CHEQUE').AsString;
{
