...
type Registro2 = class
    ...
    Reservado_2:String;// * 2 Reservado N/A
    FechaLimiteDePago_08:String;// * 2 Fecha Limite de Pago
    ...
    NumeroRealDeTarjeta_20NA:String; // * 20 NA
    CorreoElectronico_60Op:String; // * 60 Op
    NumeroCelular_60Op:String; // * 60 Op
    ValorRealPagado_12Op:String; // * 60 Op
...
var cadena:string;
    ...
    Plazo: Integer;
    _telefono1,_telefono2,_telefono3,_telefono4, _numeroCelular : String;
    _fechalimitedepago: TDate;
    valorMora, valorSaldo: Integer;
begin
        ...
        with IBQuery1 do begin
           ...
           while not eof do begin
             ...
             with IBSClientes do begin
                  ...
                  SQL.Add('"gen$persona".EMAIL,');
                  SQL.Add('"gen$direccion".TELEFONO1,');
                  SQL.Add('"gen$direccion".TELEFONO2,');
                  SQL.Add('"gen$direccion".TELEFONO3,');
                  SQL.Add('"gen$direccion".TELEFONO4');
             ...
             _telefono1 := IBSClientes.FieldByName('TELEFONO1').AsString;
             _telefono2 := IBSClientes.FieldByName('TELEFONO2').AsString;
             _telefono3 := IBSClientes.FieldByName('TELEFONO3').AsString;
             _telefono4 := IBSClientes.FieldByName('TELEFONO4').AsString;

             _numeroCelular := '';
             if (Length(_telefono1) = 10 ) then _numeroCelular := _telefono1
             else if (Length(_telefono2) = 10 ) then _numeroCelular := _telefono2
             else if (Length(_telefono3) = 10 ) then _numeroCelular := _telefono3
             else if (Length(_telefono4) = 10 ) then _numeroCelular := _telefono4;

             LineaC.NumeroCelular_60Op := Format('%-60s',[_numeroCelular]);
             ...
             LineaC.NombreTitular_60 := Nombre;
              ...
              LineaC.Reservado_2 := Format('%-2s',[' ']);
              LineaC.NumeroRealDeTarjeta_20NA := Format('%-20s',[' ']);
              LineaC.CorreoElectronico_60Op := Format('%-60s',[IBSClientes.FieldByName('EMAIL').AsString]);


              _fechalimitedepago :=  IBQuery1.FieldByName('FECHA_INTERES').AsDateTime;
              Amortizacion := IBQuery1.FieldByName('AMORTIZA_INTERES').AsInteger;
              _fechalimitedepago := CalculoFecha(_fechalimitedepago, Amortizacion);
              LineaC.FechaLimiteDePago_08 := FormatDateTime('yyyymmdd',_fechalimitedepago);
              ...
              IBSCuotas.Close;
              IBSCuotas.SQL.Clear;
              IBSCuotas.SQL.Add('select count(*) as CUOTAS_PAGAS from "col$tablaliquidacion" WHERE ID_COLOCACION = :ID_COLOCACION and PAGADA = :PAGADA');
              ...
              begin
                IBSCuotas.Close;
                IBSCuotas.SQL.Clear;
                IBSCuotas.SQL.Add('select SUM(CAPITAL_A_PAGAR) as CAPITAL from "col$tablaliquidacion" WHERE ID_COLOCACION = :ID_COLOCACION and PAGADA = :PAGADA and FECHA_A_PAGAR <= :FECHA_A_PAGAR');
                IBSCuotas.ParamByName('ID_COLOCACION').AsString := IBQuery1.FieldByName('ID_COLOCACION').AsString;
                IBSCuotas.ParamByName('PAGADA').AsInteger := 0;
                IBSCuotas.ParamByName('FECHA_A_PAGAR').AsDate := EdFechaCorte.Date;
                IBSCuotas.ExecQuery;
                valorMora := IBSCuotas.FieldByName('CAPITAL').AsInteger;
              end
              else
                valorMora := 0;


              valorSaldo := IBQuery1.FieldByName('DEUDA').AsInteger;
              if (valorMora > valorSaldo) then
                valorMora := valorSaldo;

              LineaC.ValorMora_12 := Format('%.12d',[valorMora div 1000]);


              LineaC.ValorSaldo_12 := Format('%.12d',[valorSaldo div 1000]);
              ...
              IBSDireccion.SQL.Add('SELECT FIRST 1 * FROM "gen$direccion" WHERE ID_IDENTIFICACION = :ID_IDENTIFICACION and ID_PERSONA = :ID_PERSONA and CONSECUTIVO = :CONSECUTIVO');
              ...
              IBSDireccion.ParamByName('CONSECUTIVO').AsInteger := 1;
              ...
              LineaC.ValorRealPagado_12Op := Format('%.12d',[(IBQuery1.FieldByName('VALOR').AsInteger - IBQuery1.FieldByName('DEUDA').AsInteger) div 1000]);
              ...
              with IBSCodeudores do begin
                  ...
                  while not IBSCodeudores.Eof do begin
                   ...
                   LineaO.Reservado_2 := Format('%-2s',[' ']);
                   LineaO.NumeroRealDeTarjeta_20NA := Format('%-20s',[' ']);
                   LineaO.CorreoElectronico_60Op := Format('%-60s',[IBSCodeudores.FieldByName('EMAIL').AsString]);
                    ...
                    _telefono1 := IBSDireccion.FieldByName('TELEFONO1').AsString;
                    _telefono2 := IBSDireccion.FieldByName('TELEFONO2').AsString;
                    _telefono3 := IBSDireccion.FieldByName('TELEFONO3').AsString;
                    _telefono4 := IBSDireccion.FieldByName('TELEFONO4').AsString;
...
begin
    Result := TRegistro_1+TIdentificacion_2+NumeroIdentificacion_15+NombreTitular_60+Reservado_2+FechaLimiteDePago_08+NumeroObligacion_20+CodigoSucursal_6+Calidad_1+
              ...
              TipoDeudaCartera_4NA+TipoPoliza_4NA+CodigoRamo_6NA+NumeroRealDeTarjeta_20NA+CorreoElectronico_60Op+NumeroCelular_60Op+ValorRealPagado_12Op;
end;
        ...
        with IBQuery1 do begin
           ...
           while not eof do begin
             ...
             LineaC.NombreTitular_60 := Nombre;
              ...
              IBSDireccion.SQL.Clear;
              IBSDireccion.SQL.Add('SELECT * FROM "gen$direccion" WHERE ID_IDENTIFICACION = :ID_IDENTIFICACION and ID_PERSONA = :ID_PERSONA and CONSECUTIVO = :CONSECUTIVO');
              IBSDireccion.ParamByName('ID_IDENTIFICACION').AsInteger := IBQuery1.FieldByName('ID_IDENTIFICACION').AsInteger;
              IBSDireccion.ParamByName('ID_PERSONA').AsString := IBQuery1.FieldByName('ID_PERSONA').AsString;
              IBSDireccion.ParamByName('CONSECUTIVO').AsInteger := 1;
              IBSDireccion.ExecQuery;
              ...
              LineaC.TipoPoliza_4NA := Format('%-4s',[' ']);
              LineaC.CodigoRamo_6NA := Format('%-6s',[' ']);
              LineaC.ValorRealPagado_12Op := Format('%.12d',[IBQuery1.FieldByName('ABONOS_CAPITAL').AsInteger div 1000]);
              ...
              with IBSCodeudores do begin
               ...
               while not IBSCodeudores.Eof do begin
                ...
                LineaC.Reservado_2 := Format('%-2s',[' ']);
                LineaC.NumeroRealDeTarjeta_20NA := Format('%-20s',[' ']);
                LineaC.CorreoElectronico_60Op := Format('%-60s',[IBSCodeudores.FieldByName('EMAIL').AsString]);
