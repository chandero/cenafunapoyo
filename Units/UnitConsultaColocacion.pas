...
type
  TfrmConsultaColocacion = class(TForm)
    ...
    btnCambiarAsesor: TButton;
    IBQueryChangeAsesor: TIBQuery;
    ...
    procedure btnCambiarAsesorClick(Sender: TObject);
  private
...
begin
        ...
        begin
                ...
                SQL.Add('LEFT JOIN COLOCACIONASESOR ON COLOCACIONASESOR.ID_COLOCACION = "col$colocacion".ID_COLOCACION');
                SQL.Add('LEFT JOIN ASESOR ON ASESOR.ASES_ID = COLOCACIONASESOR.ASES_ID');
                SQL.Add(' WHERE ');
               ...
               // ParamByName('ID_AGENCIA').AsInteger := Agencia;
                ...
                begin
                      ...
                      EdAgencia.KeyValue := FieldByName('ID_AGENCIA').AsString;
                      ...
                      DBListAsesor.KeyValue := FieldByName('ASES_ID').AsInteger;
...
begin
            if IBQAsesor.Transaction.InTransaction then
              IBQAsesor.Transaction.Rollback;
            IBQAsesor.Transaction.StartTransaction;
...
begin
        ...
        IBQueryChangeAsesor.Database := dmGeneral.IBDatabase1;
        IBTransaction1.DefaultDatabase := dmGeneral.IBDatabase1;

        IBQueryChangeAsesor.Transaction := IBTransaction1;

        //if control_consulta then
        //   dmColocacion := TdmColocacion.Create(Self);
end;

procedure TfrmConsultaColocacion.btnCambiarAsesorClick(Sender: TObject);
begin
        IBQueryChangeAsesor.Close;
        IBQueryChangeAsesor.SQL.Clear;
        IBQueryChangeAsesor.SQL.Add('UPDATE COLOCACIONASESOR ca SET ca.ASES_ID = :ASES_ID WHERE ca.ID_COLOCACION = :ID_COLOCACION');
        IBQueryChangeAsesor.ParamByName('ASES_ID').AsInteger := DBListAsesor.KeyValue;
        IBQueryChangeAsesor.ParamByName('ID_COLOCACION').AsString := Colocacion;

        if IBTransaction1.InTransaction then
           IBTransaction1.Commit;
        IBTransaction1.StartTransaction;

        IBQueryChangeAsesor.ExecSQL;
        IBTransaction1.Commit;

        ShowMessage('Se realizó el Cambio de Asesor');
end;
