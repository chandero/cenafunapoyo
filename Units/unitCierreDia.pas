...
type
  Tfrmcierredia = class(TForm)
    Panel1: TPanel;
    txtFechaInicial: TDateTimePicker;
    Label1: TLabel;
    ...
    procedure txtFechaInicialKeyPress(Sender: TObject; var Key: Char);
...
begin
   ...
   with querycompcierre do
    begin
      ...
      sql.Add('where "con$comprobante"."FECHADIA" BETWEEN :FECHAINICIAL and :FECHAFINAL and "con$comprobante".ESTADO = :ESTADO');
      ParamByName('FECHAINICIAL').AsDate := txtFechaInicial.Date;
      ParamByName('FECHAFINAL').AsDate := txtFechaFinal.Date;
      ...
      if querycompcierre.RecordCount = 0 then
       begin
         MessageDlg('No exiten comprobantes para cerrar en el rango',mterror,[mbok],0);
         exit;
       end;
...
// Verificar Cuenta
                       ...
                       IBSQLcierre.SQL.Add('select count(*) as total from "con$puc" where CODIGO = :CODIGO');
                       IBSQLcierre.ParamByName('CODIGO').AsString := codigo;
                       try
                        IBSQLcierre.ExecQuery;
                        if IBSQLcierre.FieldByName('total').AsInteger < 1 then
...
begin
        lerror := Tlist.Create;
        txtFechaInicial.Date := fFechaActual;
        txtFechaFinal.Date := txtFechaInicial.Date;
end;
         ...
         begin
           with IBQRepnormal do
            begin
              ...
              sql.Add('from "con$comprobante"');
              sql.Add('where ');
              sql.Add('"con$comprobante"."FECHADIA" BETWEEN :FECHAINICIAL and :FECHAFINAL and');
              sql.Add('"con$comprobante"."ESTADO" =:"ESTADO"');
              ParamByName('FECHAINICIAL').AsDate := txtFechaInicial.Date;
              ParamByName('FECHAFINAL').AsDate := txtFechaFinal.Date;
              ParamByName('ESTADO').AsString := 'C';
        ...
        if vcierre = False then
         begin
           with IBQRepanormal do
            begin
              sql.Clear;
              sql.Add('Select e.ID_AGENCIA, t.DESCRIPCION,');
              sql.Add('e.ID_COMPROBANTE,');
              sql.Add('e.ERROR');
              sql.Add('from "con$errorcierrecomp" e');
              SQL.Add('INNER JOIN "con$tipocomprobante" t ON e.TIPO_COMPROBANTE = t.ID');
              open;
            end;
            Repanormal.Variables.ByName['empresa'].AsString := empresa;
            Repanormal.Variables.ByName['fechainicial'].AsString := datetostr(txtFechaInicial.Date);
            Repanormal.Variables.ByName['fechafinal'].AsString := datetostr(txtFechaFinal.Date);
...
procedure Tfrmcierredia.txtFechaInicialKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self);
end;
...
begin
  ...
  _queryUpd.SQL.Add('where ');
  _queryUpd.SQL.Add('ID_COMPROBANTE = :ID_COMPROBANTE and');
