...
uses
  ...
  JvCheckedMaskEdit, JvDatePickerEdit, IBStoredProc, IBDatabase;
...
type
  TfrmLiquidacionCarteraJudicial = class(TForm)
    ...
    Label13: TLabel;
    EdCartas: TJvCurrencyEdit;
    Report: TprTxReport;
    edFecha: TDateTimePicker;
    dblAbogado: TDBLookupComboBox;
    Label14: TLabel;
    IBQabogado: TIBQuery;
    ...
    procedure EdCartasExit(Sender: TObject);
  private
...
uses UnitGlobales, UnitBuscarColocacion,
     UnitVistaPreliminar, UnitGlobalesCol, UnitFacturar;
...
begin
        ...
        IBQAbogado.Database := dmGeneral.IBDatabase1;
        ...
        IBSQLAux.Transaction := IBTransaction1;
        IBQAbogado.Transaction := dmGeneral.IBTransaction1;
...
begin
        ...
        TGeneral := vAbonoCapital + vAbonoInteresCausado + vAbonoInteresContingente + vAbonoInteresCorriente +
                    ...
                    vComisionBanco + vSeguroVida + vCartas;
...
begin
        ...
        begin
           ...
           ExecQuery;
           CodigoCartas := FieldByName('CODIGO').AsString;
        ...
        try
          ...
          dmColocacion.IBSQLcodigosdup.Close;
          dmColocacion.IBSQLcodigosdup.SQL.Clear;
          dmColocacion.IBSQLcodigosdup.SQL.Add('select * from "col$codigospucfacturado" where ');
          dmColocacion.IBSQLcodigosdup.SQL.Add('ID_CLASIFICACION = :"ID_CLASIFICACION" and ');
          dmColocacion.IBSQLcodigosdup.SQL.Add('ID_GARANTIA = :"ID_GARANTIA" and ');
          dmColocacion.IBSQLcodigosdup.SQL.Add('ID_CATEGORIA = :"ID_CATEGORIA" ');
          dmColocacion.IBSQLcodigosdup.ParamByName('ID_CLASIFICACION').AsInteger := Clasificacion;
          dmColocacion.IBSQLcodigosdup.ParamByName('ID_GARANTIA').AsInteger      := Garantia;
          dmColocacion.IBSQLcodigosdup.ParamByName('ID_CATEGORIA').AsString     := Categoria;
          dmColocacion.IBSQLcodigosdup.ExecQuery;
...
//           GrabarExtractoDet;
           ...
           Totaldebitos := Totaldebitos + AR^.Debito;
           Totalcreditos := Totalcreditos + AR^.Credito;
// Duplicar para Causados
                    New(ARdup);
                    _codigopucdup := dmColocacion.IBSQLcodigos.FieldByName('COD_INT_MES').AsString;
                    ARdup^.CuotaNumero := AR^.CuotaNumero;
                    ARdup^.CodigoPuc :=  _codigopucdup;
                    ARdup^.FechaInicial := AR^.FechaInicial;
                    ARdup^.FechaFinal := AR^.FechaFinal;
                    ARdup^.Dias := AR^.Dias;
                    ARdup^.Tasa := AR^.Tasa;
                    ARdup^.Debito := AR^.Credito;
                    ARdup^.Credito := AR^.Debito;
                    ARdup^.EsCapital := false;
                    ARdup^.EsCausado := false;
                    ARdup^.EsCorriente := false;
                    ARdup^.EsVencido := false;
                    ARdup^.EsAnticipado := false;
                    ARdup^.EsDevuelto := false;
                    ARdup^.Id_Identificacion := 0;
                    ARdup^.Id_Persona := '';
                    MyCuotasLiq.Lista.Add(ARdup);
                    TotalCreditos := TotalCreditos + ARdup^.Credito;
                    TotalDebitos  := TotalDebitos  + ARdup^.Debito;

                    New(ARdup);
                    _codigopucdup := dmColocacion.IBSQLcodigosdup.FieldByName('COD_INT_MES').AsString;
                    ARdup^.CuotaNumero := AR^.CuotaNumero;
                    ARdup^.CodigoPuc :=  _codigopucdup;
                    ARdup^.FechaInicial := AR^.FechaInicial;
                    ARdup^.FechaFinal := AR^.FechaFinal;
                    ARdup^.Dias := AR^.Dias;
                    ARdup^.Tasa := AR^.Tasa;
                    ARdup^.Debito := AR^.Debito;
                    ARdup^.Credito := AR^.Credito;
                    ARdup^.EsCapital := false;
                    ARdup^.EsCausado := false;
                    ARdup^.EsCorriente := false;
                    ARdup^.EsVencido := false;
...
//           GrabarExtractoDet;
        ...
        if vcostas <> 0 then
         begin
           ...
           AR^.EsAnticipado := False;
           AR^.EsDevuelto := False;
           AR^.Id_Identificacion := IBQabogado.FieldByName('ID_IDENTIFICACION').AsInteger;
           AR^.Id_Persona := IBQabogado.FieldByName('ID_PERSONA').AsString;
           TasaLiquidacion := 0;
            if (AR^.Debito <> 0) or
               (AR^.Credito <> 0) then
               MyCuotasLiq.Lista.Add(AR);
...
// Coloco Comision Banco / Seguro
        ...
        begin
           ...
           AR^.EsAnticipado := False;
           AR^.EsDevuelto := False;
           AR^.Id_Identificacion := 4;
           AR^.Id_Persona := '890903790-5';
           TasaLiquidacion := 0;
            if (AR^.Debito <> 0) or
               (AR^.Credito <> 0) then
               MyCuotasLiq.Lista.Add(AR);
...
//     Verificacion de Credito
     if ChVCredito.Checked then
      ...
      if (vcostas <> 0) or (vhonorarios <> 0) then begin
        ...
        IBConsulta.ParamByName('ID_AGENCIA').AsInteger := vAgencia;
        IBConsulta.ParamByName('ID_COLOCACION').AsString := vColocacion;
...
begin
        ...
        for I := 0 to (MyCuotasLiq.Lista.Count - 1) do
         begin
              ...
              IBInforme.FieldByName('ID_AGENCIA').AsInteger := vAgencia;
...
begin
  ...
  vInteresCobrado         := 0;
  vFechaInicialAnticipado := _vFecha;
  vFechaFinalAnticipado   := _vFecha;
  vFechaInicialContingente := _vFecha;
  vFechaFinalContingente := _vFecha;
  vFechaInicialCorriente  := _vFecha;
  vFechaFinalCorriente    := _vFecha;
  vFechaInicialCausado    := _vFecha;
  vFechaFinalCausado      := _vFecha;
  vFechaInicialMora       := _vFecha;
  vFechaFinalMora         := _vFecha;
  vFechaInicialCobradoCausado := _vFecha;
  vFechaFinalCobradoCausado := _vFecha;
  vFechaInicialCobradoCorriente := _vFecha;
  vFechaFinalCobradoCorriente := _vFecha;
  vFechaInicialDevuelto   := _vFecha;
  vFechaFinalDevuelto     := _vFecha;
  vFechaInteres           := _vFecha;
  vFechaCapital           := _vFecha;
  vDiasAnticipado         := 0;
  vDiasContingente        := 0;
  vDiasCorriente          := 0;
  vDiasCausado            := 0;
  ...
  vFechaIInt              := _vFecha;
  vFechaFInt              := _vFecha;
  FechaInteres            := _vFecha;
  EdFecha.Date            := _vFecha;
  EdFecha.MinDate         := _vFecha - 7;
...
//  EdTipoIdentificacion.Caption := '';
  ...
  EdFechaInicialDevuelto.Date := _vFecha;
  EdFechaFinalDevuelto.Date := _vFecha;
  EdAbonoInteresCausado.Text := '';
  EdFechaInicialCausado.Date := _vFecha;
  EdFechaFinalCausado.Date := _vFecha;
  EdDiasAplicadosCausado.Text := '';
  EdTasaAplicadaCausado.Text := '';
  EdAbonoInteresContingente.Text := '';
  EdFechaInicialContingente.Date := _vFecha;
  EdFechaFinalContingente.Date := _vFecha;
  EdDiasAplicadosContingente.Text := '';
  EdTasaAplicadaContingente.Text := '';
  EdAbonoInteresCorriente.Text := '';
  EdFechaInicialCorriente.Date := _vFecha;
  EdFechaFinalCorriente.Date := _vFecha;
  EdDiasAplicadosCorriente.Text := '';
  EdTasaAplicadaCorriente.Text := '';
  EdAbonoInteresAnticipado.Text := '';
  EdFechaInicialAnticipado.Date := _vFecha;
  EdFechaFinalAnticipado.Date := _vFecha;
  EdDiasAplicadosAnticipado.Text := '';
  EdTasaAplicadaAnticipado.Text := '';
  EdAbonoInteresMora.Text := '';
  EdFechaInicialMora.Date := _vFecha;
  EdFechaFinalMora.Date := _vFecha;
  ...
  EdFechaConsignacion.Date := _vFecha - 7;
...
///**Genero el consecutivo
    ...
    _valorFacturarCXC := 0;
    _valorFacturarMES := 0;
    _valorFacturarMORA := 0;
    _valorFacturarANT := 0;
    _valorFacturarDEV := 0;

    Consec := FormatCurr('00000000',ObtenerConsecutivo(IBSaldo,TipoComprobante));
    vComprobante := Consec;
     ...
     try
      ...
      with IBQuery1 do
       begin
         ...
         ParamByName('ID_AGENCIA').AsInteger := vAgencia;
         ParamByName('TIPO_COMPROBANTE').AsInteger := TipoComprobante;
         if vNacional > 0 then
...
>>>>>>> 171925b3cf59501bab9dd1664befb26ff80c6cee
            ...
            if LeftStr(AR^.CodigoPuc,2) = '11' then
              ParamByName('ID_CLASE_OPERACION').AsString := '2.1.1'
            else
              ParamByName('ID_CLASE_OPERACION').AsString := '';
            ...
            ParamByName('ID').AsInteger := idauxiliar;
            if vNacional > 0 then
                ParamByName('DETALLE').AsString := 'Descuento de Cartera, Pagaré No.' + vColocacion + ' ' + 'Consig. Nal' + ' ' + DateToStr(FechaConsignacion)
            else
                ParamByName('DETALLE').AsString := 'Descuento de Cartera, Pagaré No.' + vColocacion;
            ...
            // Acumular valores para Facturar
            if AR^.EsCausado then
            begin
              _valorFacturarCXC := _valorFacturarCXC + AR^.Credito;
            end;
            if AR^.EsCorriente then
            begin
              _valorFacturarMES := _valorFacturarMES + AR^.Credito;
            end;
            if AR^.EsVencido then
            begin
              _valorFacturarMORA := _valorFacturarMORA + AR^.Credito;
            end;
            if AR^.EsAnticipado then
            begin
              _valorFacturarANT := _valorFacturarANT + AR^.Credito;
            end;
            if AR^.EsDevuelto then
            begin
              _valorFacturarDEV := _valorFacturarDEV + AR^.Debito;
            end;
...
//    verificarCancelacionCaptacion(Agencia,vColocacion);
     ...
     begin
             _vFacturaConsecutivo := ObtenerConsecutivo(50);
             _consecutivoFacturaValido := validarConsecutivoFactura(_vFacturaConsecutivo);
             if not _consecutivoFacturaValido then
             begin
                 if MessageDlg('Ya no hay facturas disponible, continuar con la liquidación', mtConfirmation, [mbYes, mbNo], 0) = mrNo then
                 begin
                   ShowMessage('Cancelando Liquidación');
                   Exit;
                 end;
             end;
...
>>>>>>> 171925b3cf59501bab9dd1664befb26ff80c6cee
                     ...
                     begin
                       SQL.Clear;
                       SQL.Add('INSERT INTO FACTURA (FACT_NUMERO, FACT_FECHA, FACT_DESCRIPCION, TIPO_COMPROBANTE, ID_COMPROBANTE, FECHA_COMPROBANTE, ID_IDENTIFICACION, ID_PERSONA, ID_EMPLEADO, FACT_ESTADO)');
                       SQL.Add(' VALUES (:FACT_NUMERO, :FACT_FECHA, :FACT_DESCRIPCION, :TIPO_COMPROBANTE, :ID_COMPROBANTE, :FECHA_COMPROBANTE, :ID_IDENTIFICACION, :ID_PERSONA, :ID_EMPLEADO, :FACT_ESTADO)');
                       ParamByName('FACT_NUMERO').AsInteger := _vFacturaConsecutivo;
                       ParamByName('FACT_FECHA').AsDateTime := _fechaFactura;
                       ParamByName('FACT_DESCRIPCION').AsString := 'Ingresos por Cartera de Crédito';
                       ParamByName('TIPO_COMPROBANTE').AsInteger := 1;
                       ParamByName('ID_COMPROBANTE').AsInteger := StrToInt(vComprobante);
                       ParamByName('FECHA_COMPROBANTE').AsDate := EdFecha.Date;
                       ParamByName('ID_IDENTIFICACION').AsInteger :=  vTipoId;
                       ParamByName('ID_PERSONA').AsString := vNumeroId;
                       ParamByName('ID_EMPLEADO').AsString := DBAlias;
                       ParamByName('FACT_ESTADO').AsInteger := 1;
                       ExecSQL;

                       Close;
                       SQL.Clear;
                       SQL.Add('INSERT INTO FACTURA_ITEM (FACT_NUMERO, FAIT_DETALLE, FAIT_CANTIDAD, FAIT_VALORUNITARIO, FAIT_TASAIVA, FAIT_VALORIVA, FAIT_TOTAL)');
                       SQL.Add(' VALUES (:FACT_NUMERO, :FAIT_DETALLE, :FAIT_CANTIDAD, :FAIT_VALORUNITARIO, :FAIT_TASAIVA, :FAIT_VALORIVA, :FAIT_TOTAL)');
                       ParamByName('FACT_NUMERO').AsInteger := _vFacturaConsecutivo;
                       ParamByName('FAIT_CANTIDAD').AsInteger := 1;
                       ParamByName('FAIT_TASAIVA').AsFloat := 0;
                       ParamByName('FAIT_VALORIVA').AsCurrency := 0;

                       if (_valorFacturarANT > 0) then
                       begin
                          ParamByName('FAIT_DETALLE').AsString := 'Ingresos Recibidos por Anticipado';
                          ParamByName('FAIT_VALORUNITARIO').AsCurrency := _valorFacturarANT - _valorFacturarDEV;
                          ParamByName('FAIT_TOTAL').AsCurrency := _valorFacturarANT - _valorFacturarDEV;
                          ExecSQL;
                       end;

                       if (_valorFacturarCXC > 0) then
                       begin
                          ParamByName('FAIT_DETALLE').AsString := 'Ingresos Recibidos Causados';
                          ParamByName('FAIT_VALORUNITARIO').AsCurrency := _valorFacturarCXC;
                          ParamByName('FAIT_TOTAL').AsCurrency := _valorFacturarCXC;
                          ExecSQL;
                       end;

                       if (_valorFacturarMES > 0) then
                       begin
                          ParamByName('FAIT_DETALLE').AsString := 'Ingresos Recibidos del Mes';
                          ParamByName('FAIT_VALORUNITARIO').AsCurrency := _valorFacturarMES;
                          ParamByName('FAIT_TOTAL').AsCurrency := _valorFacturarMES;
                          ExecSQL;
                       end;

                       if (_valorFacturarMORA > 0) then
                       begin
                          ParamByName('FAIT_DETALLE').AsString := 'Ingresos Recibidos por Mora';
                          ParamByName('FAIT_VALORUNITARIO').AsCurrency := _valorFacturarMORA;
     ...
     VerificarCancelacionCredito(vAgencia,vColocacion);
...
begin
        with IBSQL1 do begin
          ...
          SQL.Add('from "col$colocacion" where ID_COLOCACION =:ID_COLOCACION');
...
procedure TfrmLiquidacionCarteraJudicial.EdCartasExit(Sender: TObject);
begin
        vCartas := EdCartas.Value;
        TGeneral := vAbonoCapital + vAbonoInteresCausado + vAbonoInteresContingente + vAbonoInteresCorriente +
                    vAbonoInteresAnticipado + vAbonoInteresMora + vInteresCobradoCausado + vInteresCobradoCorriente - vInteresDevuelto + vPapeleria + vExAsociados + + vhonorarios + vcostas + vComisionNal +
                    vComisionBanco + vSeguroVida + vCartas;
...
procedure TfrmLiquidacionCarteraJudicial.Facturar;
var
  _facturar: TFacturar;
begin
        _facturar := TFacturar.Create;
        edRespuesta.Text := _facturar.Facturar(_vFacturaConsecutivo);
end;
