...
// Verificar Existencia en la Base de Datos
        ...
        begin
            ...
            SQL.Clear;
            SQL.Add('select * from "gen$personaextra" where ');
            SQL.Add('ID_IDENTIFICACION = :ID_IDENTIFICACION and ');
            SQL.Add('ID_PERSONA = :ID_PERSONA');
            ParamByName('ID_IDENTIFICACION').AsInteger := vTipoIdentificacion;
            ParamByName('ID_PERSONA').AsString := vIdentificacion;
            Open;
            edProveedor.Checked := InttoBoolean(FieldByName('ES_PROVEEDOR').AsInteger);
            edRut.Text := FieldByName('NUMERO_RUT').AsString;
...
begin
      // Validar Es Proveedor tiene rut
      if (edProveedor.Checked) then
      begin
          if (edRut.Text = '') then
          begin
              ShowMessage('Si es proveedor, debe informar el número del Rut');
              Exit;
          end;
      end;

      // Validar debe ternar al menos una dirección
      if vListaDirecciones.Count < 1 then
      begin
          ShowMessage('Debe ingresar al menos una dirección');
          Exit;
      end;
...
begin
      ...
      with dmPersona.IBQuery do
        begin
           ...
           SQL.Clear;
           SQL.Add('INSERT INTO "gen$personaextra" (ID_IDENTIFICACION,ID_PERSONA, ES_PROVEEDOR, NUMERO_RUT) VALUES (');
           SQL.Add(':ID_IDENTIFICACION, :ID_PERSONA, :ES_PROVEEDOR, :NUMERO_RUT)');
           ParamByName('ID_IDENTIFICACION').AsInteger := vTipoIdentificacion;
           ParamByName('ID_PERSONA').AsString := vIdentificacion;
           ParamByName('ES_PROVEEDOR').AsInteger := BooleanoToInt(edProveedor.Checked);
           ParamByName('NUMERO_RUT').AsString := edRut.Text;
           ExecSQL;
...
procedure TfrmCreacionPersona.edProveedorClick(Sender: TObject);
begin
        if (edProveedor.Checked) then
                edRut.Enabled := True
        else
...
procedure TfrmCreacionPersona.edProveedorKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key, Self);
end;
