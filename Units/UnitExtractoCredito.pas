...
uses
  ...
  IBQuery, pr_Common, pr_TxClasses, DBClient, IBDatabase, Math, Grids, DateUtils,
  XStringGrid, DBGrids, UnitDmGeneral, Menus;
...
type
  TfrmExtractoCredito = class(TForm)
    ...
    IBQExtractoDet: TIBQuery;
    ExtractoDIAS_MORA: TIntegerField;
    PopupMenu1: TPopupMenu;
    AnularAbono1: TMenuItem;
    Report: TprTxReport;
    IBQcodigo: TIBQuery;
    ...
    procedure AnularAbono1Click(Sender: TObject);
  private
    AgC : Integer;
    Col : string;
...
uses UnitVistaPreliminar, UnitGlobales, UnitGlobalesCol, UnitAnularAbono;
...
begin
    ...
    with IBQCol do begin
      ...
      if RecordCount > 0 then begin
          ...
          vValorColocacion := FieldByName('VALOR_DESEMBOLSO').AsCurrency;
...
//          if IBQCol.RecordCount > 0 then begin
           with IBQExtractoCol do begin
             ...
             SQL.Add('LEFT JOIN "col$pagoconnal" on ');
             ...
             While not IBQExtractocol.Eof do begin
                ...
                // Buscar dias de mora en la cuota
//                   IBQExtractoDet.Close;
//                   IBQExtractoDet.ParamByName('CODIGO_PUC').AsString := CodigoMora;
//                  IBQExtractoDet.ParamByName('ID_COLOCACION').AsString :=  vcolocacion;
//                   IBQExtractoDet.ParamByName('FECHA_EXTRACTO').AsDateTime := IBQExtractocol.FieldByName('FECHA_EXTRACTO').AsDateTime;
//                   IBQExtractoDet.ParamByName('ID_CBTE_COLOCACION').AsInteger := IBQExtractocol.FieldByName('ID_CBTE_COLOCACION').AsInteger;
//                   IBQExtractoDet.Open;
//                   DiasMora := IBQExtractoDet.FieldByName('DIAS').AsInteger;
                   DiasMora := 0;
                   IF (IBQExtractoCol.FieldByName('ABONO_MORA').AsCurrency > 0) then
                   begin
                           DiasMora := DaysBetween(IBQExtractocol.FieldByName('FECHA_A_PAGAR').AsDateTime, IBQExtractocol.FieldByName('FECHA_EXTRACTO').AsDateTime);
                           //DiasMora := DiasMora - 1;
                           if (DiasMora < 0) Then DiasMora := 0;
                   end;
                //
                 Extracto.FieldByName('DIAS_MORA').AsInteger := DiasMora;
...
begin
        ...
        IBQExtractoDet.Database := dmGeneral.IBDatabase1;
...
var I:Integer;
    ...
    Cuenta:string;
    Funcionario:string;
    Asociado, Cuotas, Separador:string;
begin
         ...
         begin
           ...
           if RecordCount > 0 then
            begin
              ...
              Close;
              SQL.Clear;
              SQL.Add('SELECT e.CUOTA_EXTRACTO FROM "col$extracto" e WHERE e.ID_COLOCACION = :ID_COLOCACION and e.ID_CBTE_COLOCACION = :ID_CBTE_COLOCACION');
              ParamByName('ID_COLOCACION').AsString := vColocacion;
              ParamByName('ID_CBTE_COLOCACION').AsString := vComprobante;
              Open;
              ...
              while not Eof do
              begin
                Cuotas := Cuotas + Separador + FieldByName('CUOTA_EXTRACTO').AsString;
                Separador := ',';
                Next;
              end;
              ...
              with IBQuery do
               begin
                 ...
                 SQL.Add('LEFT JOIN "con$puc" ON ("col$extractodet".CODIGO_PUC = "con$puc".CODIGO)');
                 ...
                 Report.Variables.ByName['Cuotas'].AsString := Cuotas;
...
procedure TfrmExtractoCredito.AnularAbono1Click(Sender: TObject);
var
...
begin
  _vCodigoAprobacion := leerCodigoAprobacion(1);
  ...
  begin
      // Inactivar código de aprobación
         ...
         IBQcodigo.Database := dmGeneral.IBDatabase1;
         IBQcodigo.Transaction := _ibtransaction;
      // Inactivar código de aprobación
         if _ibtransaction.InTransaction then _ibtransaction.Commit;
         _ibtransaction.StartTransaction;
         IBQcodigo.Close;
         ...
         IBQcodigo.ParamByName('TIPO').AsInteger := 1;
         IBQcodigo.ExecSQL;
         _ibtransaction.Commit;

      //validar si es último abono
      if (Extracto.RecNo <> Extracto.RecordCount) then
      begin
             ShowMessage('Solo se puede anular el último abono, si desea anular un abono anterior, debe anular desde el último');
      end
      else
      begin
            ...
            if _ibtransaction.InTransaction then _ibtransaction.Commit;
            _ibtransaction.StartTransaction;
            frmAnularAbono := TfrmAnularAbono.Create(self);


            _ibquery.SQL.Clear;
            _ibquery.SQL.Add('select FIRST 2 * from "col$extracto" where ID_COLOCACION = :ID_COLOCACION order by FECHA_EXTRACTO DESC, HORA_EXTRACTO DESC');
            _ibquery.ParamByName('ID_COLOCACION').AsString := EdNumeroColocacion.Text;
            _ibquery.Open;
            _ibquery.Last;
            _ibquery.First;
            frmAnularAbono.Colocacion := EdNumeroColocacion.Text;
            frmAnularAbono.Nombre := EdAsociado.Caption;
            frmAnularAbono.Fecha := _ibquery.FieldByName('FECHA_EXTRACTO').AsDateTime;
            frmAnularAbono.Comprobante := _ibquery.FieldByName('ID_CBTE_COLOCACION').AsInteger;
            frmAnularAbono.AbonoCapital := _ibquery.FieldByName('ABONO_CAPITAL').AsCurrency;
            frmAnularAbono.AbonoCausado := _ibquery.FieldByName('ABONO_CXC').AsCurrency;
            frmAnularAbono.AbonoAnticipado := _ibquery.FieldByName('ABONO_ANTICIPADO').AsCurrency;
            frmAnularAbono.AbonoServicio := _ibquery.FieldByName('ABONO_SERVICIOS').AsCurrency;
            frmAnularAbono.FechaCapital := _ibquery.FieldByName('CAPITAL_PAGO_HASTA').AsDateTime;
            frmAnularAbono.FechaInteres := _ibquery.FieldByName('INTERES_PAGO_HASTA').AsDateTime;
            frmAnularAbono.Saldo := vSaldoActual;
            vSaldo := _ibquery.FieldByName('SALDO_ANTERIOR_EXTRACTO').AsCurrency;
            if (_ibquery.RecordCount > 1) then
            begin
                _ibquery.Next;
                vFechaCapital := _ibquery.FieldByName('CAPITAL_PAGO_HASTA').AsDateTime;
                vFechaInteres := _ibquery.FieldByName('INTERES_PAGO_HASTA').AsDateTime;
            end
            else
            begin
                vSaldo := vValorColocacion;
                vFechaCapital := StrToDate(EdFechaColocacion.Caption);
                vFechaInteres := StrToDate(EdFechaColocacion.Caption);
            end;
            frmAnularAbono.NuevoSaldo := vSaldo;
            frmAnularAbono.NuevaFechaCapital := vFechaCapital;
            frmAnularAbono.NuevaFechaInteres := vFechaInteres;
            _ibquery.Transaction.commit;
            _ibquery.Close;

            frmAnularAbono.ShowModal;
            MostrarExtracto;
        end;
      end
      else
