...
type
  ...
  private
    ...
    function ValidarCodigoCapital(Codigo: String): Boolean;
...
function TfrmCalculoCuotadePrueba.ValidarCodigoCapital(Codigo: String): Boolean;
var
  IBQcodigocapital: TIBQuery;
begin
        IBQcodigocapital := TIBQuery.Create(self);
        IBQcodigocapital.Database := dmGeneral.IBDatabase1;
        IBQcodigocapital.Transaction := dmGeneral.IBTransaction1;
        IBQcodigocapital.SQL.Clear;
        IBQcodigocapital.SQL.Add('SELECT COUNT(*) AS CANTIDAD FROM "col$codigospuc" WHERE COD_CAPITAL_CP = :CODIGO or COD_CAPITAL_LP = :CODIGO');
        IBQcodigocapital.ParamByName('CODIGO').AsString := Codigo;
        IBQcodigocapital.Open;
        if (IBQcodigocapital.RecordCount > 0) then
          if (IBQcodigocapital.FieldByName('CANTIDAD').AsInteger > 0) then
             result := true
          else
             result := false;
...
{                      if FieldByName('TASA_INTERES_CORRIENTE').AsFloat < FieldByName('VALOR_ACTUAL_TASA').AsFloat then
                      ...
                      IBQuery1.SQL.Add('ID_COLOCACION = :"ID_COLOCACION" and');
...
var CuotasLiq:TCuotasLiq;
    ...
    vAdicionalCapital: Currency;
    EsCodigoCapital: Boolean;
begin
 ...
 if EdNacional.Value > 0 then begin
  ...
  with IBSaldo do
   begin
     ...
     ExecQuery;
     CodigoSobrantes := FieldByName('CODIGO').AsString;
     ...
     for I := 0 to CuotasLiq.Lista.Count - 1 do
      begin
        ...
        EsCodigoCapital := ValidarCodigoCapital(AF^.CodigoPuc);
        if EsCodigoCapital then
        begin
          vTotalCapital := vTotalCapital + AF^.Credito;
        end;
...
// Es pago en Efectivo
      ...
      begin
        if vTotalLiquidacion < EdEfectivo.Value then
         begin
           vDiferencia := EdEfectivo.Value - vTotalLiquidacion;
           vMenorValor := vTotalCapital + vDiferencia;
           if (vMenorValor > SaldoActual) then
           begin
               vAdicionalCapital := SaldoActual - vTotalCapital;
               vSobrante := vDiferencia - vAdicionalCapital;
           end
           else
           begin
               vAdicionalCapital := vDiferencia;
               vSobrante := 0;
           end;
...
// Es pago por Bancos
      ...
      begin
        ...
        if vTotalLiquidacion < (EdNacional.Value - vComisionNal) then
         begin
           ...
           vDiferencia := (EdNacional.Value - vComisionNal) - vTotalLiquidacion;
           vMenorValor := vTotalCapital + vDiferencia;
           if (vMenorValor > SaldoActual) then
           begin
               vAdicionalCapital := SaldoActual - vTotalCapital;
      ...
      begin
        ...
        if vTotalLiquidacion < EdLibranza.Value then
         begin
           vDiferencia := (EdLibranza.Value) - vTotalLiquidacion;
           vMenorValor := vTotalCapital + vDiferencia;
           if (vMenorValor > SaldoActual) then
           begin
               vAdicionalCapital := SaldoActual - vTotalCapital;
...
//**
  if CuotasLiq.Liquidado then
   begin
     ...
     frmMuestroLiquidacionColocacion.Agencia := EdAgencia.KeyValue;
     frmMuestroLiquidacionColocacion.AgenciaDescripcion := EdAgencia.Text;
     ...
     frmMuestroLiquidacionColocacion.AbonoSobrante := vSobrante;
