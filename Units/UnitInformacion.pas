...
uses
  ...
  lmdstdcA,
  Unitdmgeneral,
  UnitDmSolicitud, Mask, JvToolEdit, FileCtrl
  ;
...
type
  TFrmInformacion = class(TForm)
    ...
    DBasesor: TDBLookupComboBox;
    DSasesor: TDataSource;
    IBQasesor: TIBQuery;
    TabAdjuntos: TTabSheet;
    Panel13: TPanel;
    Panel14: TPanel;
    edFilename: TJvFilenameEdit;
    btnAgregarAdjunto: TBitBtn;
    btnEliminarAdjunto: TBitBtn;
    Label101: TLabel;
    DBAdjunto: TDBGrid;
    CDSadjunto: TClientDataSet;
    CDSadjuntoRuta: TStringField;
...
begin
        ...
        _numero_radicado := numero_radicado;           
        with DmSolicitud.IBSolicitud1 do
        begin
          Close;
...
begin
        IBSQL1.Close;
        IBSQL1.SQL.Clear;
        IBSQL1.SQL.Add('SELECT COLAPO_TASA FROM COL$APORTEDONACION WHERE :PLAZO BETWEEN COLAPO_PLAZO_MIN AND COLAPO_PLAZO_MAX');
        IBSQL1.ParamByName('PLAZO').AsInteger := JVplazo.Value;
        IBSQL1.ExecQuery;
        _tasa := 0;
        if (IBSQL1.RecordCount > 0) then
        begin
          _tasa := IBSQL1.FieldByName('COLAPO_TASA').AsFloat;
        end;
        JvValorAporte.Value := JVvalorprestamo.Value * _tasa;
...
//             Exit;
           ...
           IBSQL3.Close;
           IBSQL3.SQL.Clear;
           IBSQL3.SQL.Add('SELECT COUNT(*) AS TOTAL FROM "col$colocacion" WHERE ID_IDENTIFICACION = :ID_IDENTIFICACION AND ID_PERSONA = :ID_PERSONA');
           IBSQL3.ParamByName('ID_IDENTIFICACION').AsInteger := DBidentificacion.KeyValue;
           IBSQL3.ParamByName('ID_PERSONA').AsString := JvDocumento.Text;
           IBSQL3.ExecQuery;
           if IBSQL3.RecordCount > 0 then
           begin
                if IBSQL3.FieldByName('TOTAL').AsInteger = 0 then
                begin
                  _bClienteNuevo := true;
...
begin
        ...
        if ( not VarIsNull(DBoficina.KeyValue)) then
        begin
         if (DBoficina.KeyValue < 1) then
         begin
           ShowMessage('Por favor seleccione la agencia');
           DBoficina.SetFocus;
           Exit;
         end;
        end
        ...
        if ( not VarIsNull(DBasesor.KeyValue)) then
        begin
         if (DBasesor.KeyValue < 1) then
         begin
           ShowMessage('Por favor seleccione el asesor');
           DBasesor.SetFocus;
           Exit;
         end;
        end
        ...
        begin
           ShowMessage('Por favor seleccione el tipo de solicitud');
           Pagina.ActivePageIndex := 0;
           Pagina1.ActivePageIndex := 4;
           Application.ProcessMessages;
           DBLCBsolicitudtipo.SetFocus;
           Application.ProcessMessages;
           DBLCBsolicitudtipo.DropDown;
           Exit;
        end;
...
begin
        ...
        begin
          ...
          ExecSQL;
          SQL.Clear;
          SQL.Add('INSERT INTO COL$SOLICITUD_SOLICITUD_TIPO VALUES (:ID_SOLICITUD, :SOTI_ID)');
          ParamByName('ID_SOLICITUD').AsString := num_radicado;
          ...
          SQL.Clear;
          SQL.Add('delete from SOLICITUDASESOR where ID_SOLICITUD = :ID_SOLICITUD');
          ParamByName('ID_SOLICITUD').AsString := num_radicado;
          ExecSQL;
          Transaction.CommitRetaining;
          SQL.Clear;
          SQL.Add('insert into SOLICITUDASESOR(ID_AGENCIA, ID_SOLICITUD, ASES_ID) VALUES (:ID_AGENCIA, :ID_SOLICITUD, :ASES_ID)');
          ParamByName('ID_AGENCIA').AsInteger := DBoficina.KeyValue;
          ...
          ExecSQL;
          SQL.Clear;
          SQL.Add('INSERT INTO COL$SOLICITUDAPORTE VALUES (:ID_SOLICITUD, :VALOR_APORTE, :PLAZO)');
          ParamByName('ID_SOLICITUD').AsString := num_radicado;
...
begin
        ...
        begin
          ...
          if Transaction.InTransaction then
             ...
             Transaction.StartTransaction;
          SQL.Clear;
          SQL.Add('SELECT s.*, a.* FROM');
          SQL.Add('"col$solicitud" s ');
          SQL.Add('LEFT JOIN SOLICITUDASESOR a ON a.ID_AGENCIA = s.OFICINA and a.ID_SOLICITUD = s.ID_SOLICITUD');
          SQL.Add('WHERE');
          SQL.Add('(s.ID_SOLICITUD = :ID_SOLICITUD)');
          ParamByName('ID_SOLICITUD').AsString := num_radicado;
          Open;
          DBidentificacion.KeyValue := FieldByName('ID_IDENTIFICACION').AsInteger;
          DBasesor.KeyValue := FieldByName('ASES_ID').AsInteger;
...
procedure TFrmInformacion.guardarAdjunto;
var
 _rutadestino : String;
begin
        CDSadjunto.First;
        ...
        begin
            ...
            Transaction.StartTransaction;
            SQL.Clear;
            SQL.Add('SELECT * FROM GENERAL WHERE GENE_ID = 1');
            Open;
            _rutadestino := FieldByName('GENE_CADENA').AsString;
            Close;
        end;
        if not DirectoryExists(_rutadestino + '\' + _numero_radicado) then
           if not CreateDir(_rutadestino + '\' + _numero_radicado) then
                raise Exception.Create('No se pudo crear la carpera: ' + _rutadestino + '\' + _numero_radicado);
        while not CDSadjunto.Eof do
        begin
          with DmSolicitud.IBSolicitud1 do
          begin
            Close;
...
procedure TFrmInformacion.btnAgregarAdjuntoClick(Sender: TObject);
begin
        CDSadjunto.Append;
        CDSadjuntoRuta.Value := edFilename.FileName;
        CDSadjuntoNombre.Value := edFilename.ShortName;
        CDSadjunto.Post;
        CDSadjunto.Last;
        edFilename.Clear;
end;

procedure TFrmInformacion.CDSadjuntoAfterScroll(DataSet: TDataSet);
begin
        // CDSadjunto.Last;
        if (CDSadjunto.RecordCount > 0) then
        begin
...
procedure TFrmInformacion.btnEliminarAdjuntoClick(Sender: TObject);
begin
        CDSadjunto.Delete;
end;
...
procedure TFrmInformacion.JVvalorprestamoChange(Sender: TObject);
begin

        if self.Showing then calcularAporte;
end;

procedure TFrmInformacion.JVplazoChange(Sender: TObject);
begin
        if self.Showing then calcularAporte;
end;
