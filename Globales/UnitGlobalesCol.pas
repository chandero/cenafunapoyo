...
procedure CrearTablaLiquidacionPTmp_Mod(TableName:String;vDesembolso:Currency;var ValorCuota:Currency;
                               ...
                               AmortizaInteres:Integer;Plazo:Integer;
                               TipoCuota:String;Modalidad:string;PGracia:Integer);
procedure CalcularDescuentoPorCuota(CdsCuotas, CdsDescuento, CdsADescontar:TClientDataSet; vDesembolso:Currency; AmortizaK: Integer; SaldoActual: Currency);
...
begin
  ...
  with IBSQL do
   begin
     SQL.Clear;
     SQL.Add('select * from "col$colocacion" where ID_COLOCACION = :ID_COLOCACION');
     ...
     vDesembolso := Saldo;
      ...
      Close;
      SQL.Clear;
      SQL.Add('SELECT APORTE, PLAZO  FROM COL$APORTEVINCULADO WHERE ID_COLOCACION = :ID_COLOCACION');
      ParamByName('ID_COLOCACION').AsString := IdColocacion;
      ExecQuery;
  ...
  with IBSQLcodigosdup do
   begin
     SQL.Clear;
     SQL.Add('select * from "col$codigospucfacturado" where ');
     SQL.Add('ID_CLASIFICACION = :"ID_CLASIFICACION" and ');
     SQL.Add('ID_GARANTIA = :"ID_GARANTIA" and ');
     SQL.Add('ID_CATEGORIA = :"ID_CATEGORIA" ');
     ParamByName('ID_CLASIFICACION').AsInteger := Clasificacion;
...
//              TasaMaxima := SimpleRoundTo(1.5 * (tasanominalvencida(TasaEfectiva1,Amortizacion));
          ...
          { if (Clasificacion = 1) or (Clasificacion = 2) or (Clasificacion = 4) then
            ...
            if (AF^.Anticipado) or (AF^.Devuelto) then
              AR^.CodigoPuc   := IBSQLcodigosdup.FieldByName('COD_INT_ANT').AsString
            else
             if AF^.Causado then
               AR^.CodigoPuc   := IBSQLcodigos.FieldByName('COD_CXC').AsString
            else
             if AF^.Corrientes then
               AR^.CodigoPuc   := IBSQLcodigosdup.FieldByName('COD_INT_MES').AsString
            else
             if AF^.Vencida then
               AR^.CodigoPuc   := IBSQLcodigosdup.FieldByName('COD_INT_MORA').AsString;
...
//                 ParamByName('CODIGO_PUC').AsString := IBSQLcodigos.FieldByName('COD_INT_ANT').AsString;
                 ParamByName('CODIGO_PUC').AsString := IBSQLcodigosdup.FieldByName('COD_INT_MES').AsString;
                 ExecQuery;
...
//            AR^.Dias := DaysBetween(AF^.FechaFinal,AF^.FechaInicial);
            ...
            TotalDebito  := TotalDebito  + AR^.Debito;
                 ...
                 begin
                    New(ARdup);
                    _codigopucdup := IBSQLcodigos.FieldByName('COD_INT_MES').AsString;
                    ...
                    ARdup^.EsOtros := false;
                    ARdup^.Id_Clase_Operacion := '';
                    MyCuotasLiq.Lista.Add(ARdup);
                    TotalCredito := TotalCredito + ARdup^.Credito;
                    TotalDebito  := TotalDebito  + ARdup^.Debito;

                    New(ARdup);
                    _codigopucdup := IBSQLcodigosdup.FieldByName('COD_INT_MES').AsString;
                    ARdup^.CuotaNumero := AR^.CuotaNumero;
                    ARdup^.CodigoPuc :=  _codigopucdup;
                    ARdup^.FechaInicial := AR^.FechaInicial;
                    ARdup^.FechaFinal := AR^.FechaFinal;
...
var I,J:Integer;
    ...
    IBSQL, IBSQLcodigos, IBSQLcodigosdup : TIBSQL;
    IBQuery: TIBQuery;
    dmGeneral: TdmGeneral;
begin
    dmGeneral := TdmGeneral.Create(nil);
...
//                    if CuotasLiq.Lista.Count <= 0 then Break;
        ...
        begin
           ...
           if (_fechaFinal >= FechaHoy) then Result.Dias := 0;
        end;
        IBQvalidar.Close;
        IBT.Commit;
end;
...
begin
        ...
        begin
            // Buscar Descuento
               ...
               nAmortiza := dmGeneral.IBQuery1.FieldByName('AMORTIZACION').AsInteger;
            ...
            begin
              if bEnDesembolso then Exit;
              nCobro := vDesembolso;
              nCobro := SimpleRoundTo (vDesembolso * nPorcColocacion/100,0)*(AmortizaK/nAmortiza);
            ...
            begin
              ...
              if (bEnCuota) and (not bEsDistribuido) then begin
               ...
               while not CdsCuotas.Eof do
               begin
                   nCobro := SimpleRoundTo(nValor * nPorcSaldo/100,0)*(AmortizaK/nAmortiza);
             ...
             begin
               ...
               while not CdsCuotas.Eof do
               begin
                   nCobro := nCobro + SimpleRoundTo(nValor * nPorcSaldo/100,0)*(AmortizaK/nAmortiza);
                   nValor := nValor - CdsCuotas.FieldByName('CAPITAL_A_PAGAR').AsCurrency;
                   CdsCuotas.Next;
               end;
