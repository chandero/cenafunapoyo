...
type TRDsaValor = Record
      tipo: Integer;
      tasa: Currency;
      valor: Currency;
end;

TRDsaValorArray = array of TRDsaValor;
...
function  leerCodigoAprobacion(tipo:Integer): String;
...
function GetSMSCredentials: TSmsCredentials;
function getPeriodos:TDataSource;
...
function ObtenerConsecutivo(IBSQL1:TIBSQL; _iTipo:Integer): LongInt; Overload;
function ObtenerConsecutivo(_id:Integer): LongInt; Overload;
function ObtenerConsecutivoAuxiliar(IBSQL1:TIBSQL): LongInt;
...
function validarConsecutivoDSA(_id: Integer): Boolean;
...
begin
    ...
    _conn := TdmGeneral.Create(nil);
    _conn.getConnected;

    IBQ.Database := _conn.IBDatabase1;
    IBT.DefaultDatabase := _conn.IBDatabase1;

    IBT.StartTransaction;

    IBQ.SQL.Clear;
    IBQ.SQL.Add('SELECT DEPA_NOMBRE FROM DEPARTAMENTO WHERE DEPA_ID = :DEPA_ID');
    IBQ.ParamByName('DEPA_ID').AsString := depa_id;
    IBQ.Open;

    result := IBQ.FieldByName('DEPA_NOMBRE').AsString;
    IBQ.Close;
    IBT.Commit;
...
begin
    ...
    IBQ.SQL.Clear;
    IBQ.SQL.Add('SELECT FIRST 1 * FROM GEN$CODIGOAPROBACION WHERE COAP_ESTADO = 0 AND COAP_TIPO = :TIPO ORDER BY COAP_GENERADO DESC');
    IBQ.ParamByName('TIPO').AsInteger := tipo;
    IBQ.Open;

    result := IBQ.FieldByName('COAP_CODIGO').AsString;
    IBQ.Close;
    IBT.Commit;
...
begin
        ...
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 1;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Enero';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 2;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Febrero';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 3;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Marzo';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 4;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Abril';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 5;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Mayo';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 6;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Junio';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 7;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Julio';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 8;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Agosto';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 9;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Septiembre';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 10;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Octubre';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 11;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Noviembre';
        CDSdata.Post;
        CDSdata.Insert;
        CDSdata.FieldByName('ID').AsInteger := 12;
        CDSdata.FieldByName('DESCRIPCION').AsString := 'Diciembre';
        CDSdata.Post;
        CDSdata.IndexFieldNames := 'ID';
...
begin
       ...
       Result := TSmsCredentials.Create();
        with IBSQL1 do begin
         ...
         try
          SQL.Clear;
          SQL.Add('select FIRST 1 * from "gen$sms"');
          ExecQuery;
          Result.url  := FieldByName('URL').AsString;
          Result.usuario := FieldByName('USUARIO').AsString;
          Result.clave := FieldByName('CLAVE').AsString;
          Close;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          DmComprobante.IBVarios.SQL.Add('"con$saldoscuenta".CODIGO = :CODIGO');
          DmComprobante.IBQuery2.SQL.Clear;
          DmComprobante.IBQuery2.SQL.Add('update "con$puc" SET SALDOINICIAL = 0 where');
          DmComprobante.IBQuery2.SQL.Add('"con$puc".CODIGO = :CODIGO');
...
begin
        with IBVarios do begin
          Close;
          SQL.Clear;
          SQL.Add('select "con$puc".NIVEL,"con$puc".CODIGOMAYOR from "con$puc" where CODIGO = :CODIGO');
        ...
        with ibvarios do begin
           ...
           SQL.Add('where CODIGO = :CODIGO and MES = :MES');
           ParamByName('CODIGO').AsString := CodigoMayor;
...
begin
       ...
       _ibSQL1.Transaction := _ibTransaction;
       ...
       with _ibSQL1 do begin
         ...
         Transaction.StartTransaction;
          ...
          Close;
          SQL.Clear;
          SQL.Add('SELECT VALOR_MINIMO FROM "gen$minimos" WHERE ID_MINIMO = 110');
          Open;
          MinFacturas := FieldByName('VALOR_MINIMO').AsInteger;
          Close; 
       end;



       if (_id + MinFacturas) >= Consecutivo then
       begin
          ShowMessage('Ya casi se acaban las facturas, faltan:' + IntToStr(Consecutivo - _id));
       end;
...
begin
       ...
       with _ibSQL1 do begin
         ...
         Transaction.StartTransaction;
          SQL.Clear;
          SQL.Add('select FIRST 1 DSAU_SECUENCIAFINAL from DSA_AUTORIZACION ORDER BY DSAU_ID DESC');
          Open;
          Consecutivo := FieldByName('DSAU_SECUENCIAFINAL').AsInteger;
          Close;
       end;

       if (_id + 30) >= Consecutivo then
       begin
          ShowMessage('Ya casi se acaba la n�meraci�n de Documento Soporte de Adquisici�n, faltan:' + IntToStr(Consecutivo - _id));
       end;
...
function ObtenerConsecutivo(_id:Integer): LongInt;
const ntMaxTries = 100;
var I, WaitCount, Tries,Consecutivo:Integer;
    RecordLocked:Boolean;
    ErrorMsg:string;
...
begin
       ...
       while True do
        with _ibSQL1 do begin
         ...
         try
          SQL.Clear;
          SQL.Add('select CONSECUTIVO from "gen$consecutivos" WHERE ID_CONSECUTIVO = :ID_CONSECUTIVO');
          ParamByName('ID_CONSECUTIVO').AsInteger := _id;
          ExecQuery;
...
begin
       ...
       while True do
        with IBSQL1 do begin
         ...
         try
          SQL.Clear;
          SQL.Add('select * from "gen$consecutivos" where "gen$consecutivos"."ID_CONSECUTIVO" = 10');
          ExecQuery;
          Consecutivo := FieldByName('CONSECUTIVO').AsInteger;
          Close;
          Consecutivo := Consecutivo + 1;
          SQL.Clear;
          SQL.Add('update "gen$consecutivos" set "gen$consecutivos"."CONSECUTIVO" = :"CONSECUTIVO" ');
          SQL.Add(' where "gen$consecutivos"."ID_CONSECUTIVO" = 10');
          ParamByName('CONSECUTIVO').AsInteger := Consecutivo;
          ExecQuery;
          Transaction.Commit;
          Result := Consecutivo;
        ...
        with IBSQL1 do begin
           ...
           try
             ...
             if RecordCount < 1 then begin
                MessageDlg('Usted no est� autorizado para ejecutar esta opci�n',mtError,[mbcancel],0);
                Result := False;
             end
             else
                Result := True;
