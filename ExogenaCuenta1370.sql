SELECT 1370 as CODIGO, 1315 AS CONCEPTO, IIF(c.ID_IDENTIFICACION = 3,13,31) AS ID, c.ID_PERSONA, '' AS DIV, p.PRIMER_APELLIDO, p.SEGUNDO_APELLIDO, p.NOMBRE, '' AS SNOMBRE, '' AS RAZON,  d.DIRECCION, SUBSTRING(d.COD_MUNICIPIO FROM 1 FOR 2) AS DPTO, SUBSTRING(d.COD_MUNICIPIO FROM 3 FOR 3) AS MUNICIPIO,  169 AS  PAIS, SUM(c.DEUDA) FROM "col$causaciondiaria" c 
